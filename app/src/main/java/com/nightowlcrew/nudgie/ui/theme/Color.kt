package com.nightowlcrew.nudgie.ui.theme

import androidx.compose.ui.graphics.Color

// Primary Brand Colors
val BrandGold = Color(0xFFFFD700)     // Primary "Win" color for XP bars
val HeartRed = Color(0xFFFF8A8A)      // Happiness stat color
val ElectricYellow = Color(0xFFFFEB3B) // Energy stat color
val LevelUpBlue = Color(0xFF29B6F6)    // Learning/Skill color
val LogicPurple = Color(0xFFE1BEE7)    // AI/Thinking dialogue color
val SuccessGreen = Color(0xFF00C853)   // Completion/Success color
val DarkGreyScale = Color(0xFF333333)  // Used for tactile depth

// UI Specific Colors from Design
val DarkBackground = Color(0xFF1F2937)
val CardRedBg = Color(0xFFFFEBEE)
val CardYellowBg = Color(0xFFFFFDE7)
val CardBlueBg = Color(0xFFE3F2FD)
val CardGreenBg = Color(0xFFE8F5E9)

// Cyberpunk Theme Colors
val cpVoid = Color(0xFF050510)         // True black/blue background
val cpMainframe = Color(0xFF0D0D17)     // Slightly lighter for cards
val cpNeonCyan = Color(0xFF00F0FF)      // Primary interaction
val cpDimCyan = Color(0xFF00B2BE)       // Readable body text
val cpNeonPink = Color(0xFFFF00FF)      // Accent/Critical alerts
val cpCorpoRed = Color(0xFFFF003C)      // Danger/Errors
val cpCyberYellow = Color(0xFFFCEE0A)   // Warning/Level stats
val cpNeonGreen = Color(0xFF00FF00)     // Success/Age stats
val cpGlitchBlue = Color(0xFF711CFF)    // Deep digital accent

// Steampunk Theme Colors
val spLeather = Color(0xFF1A1412)      // Background
val spIron = Color(0xFF2E221F)         // Surfaces/Cards
val spParchment = Color(0xFFD4B886)    // Body text/Readability
val spCopper = Color(0xFFB85D19)       // Secondary accent
val spBrass = Color(0xFFC5A059)        // Primary interaction

// Goth Theme Colors
val gothAbyss = Color(0xFF050505)      // Pitch black background
val gothObsidian = Color(0xFF16161A)   // Very dark gray for surfaces
val gothBloodRed = Color(0xFF8A0303)    // Deep, dark red
val gothNeonPink = Color(0xFFFF1493)    // High-contrast hot pink
val gothTombstone = Color(0xFFD3D3D3)   // Pale silver/gray text
